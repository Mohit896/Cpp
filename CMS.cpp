...
class Admin
{
	...
	void main()
	{
		...
		if(ch==1)
		{
			...
			do
			{
				...
				if(tmp==1) //for inserting single faculty record 
				{
					cout<<"\nEnter the Details : ";
					fstream fs;
					fs.open("fa_info.txt",ios::in|ios::out|ios::ate);
					a.faculty_data();
				...
				if(tmp==2)  // for inserting multiple faculty record
				{
					...
					fs.close();
					cout<<"\nRecord Enteeered Successfully...";
				}

				if(tmp==3)   //for view all faculty records
				{
					fstream fs;
					fs.open("fa_info.txt",ios::in);
					fs.seekg(0);
					while(!fs.eof())
					{
				...
				if(tmp==4)  //for deleting a faculty record
				{
					...
					fs.open("fa_info.txt",ios::in);
					fstream fs1;
					fs1.open("fa_new_info.txt",ios::out|ios::ate);
					while(!fs.eof())
					{
						fs.read((char *)&a,sizeof(Admin));
					...
					fs1.close();
					remove("fa_info.txt");
					rename("fa_new_info.txt","fa_info.txt");
				}
		...
		if(ch==2)
		{
			...
			cin>>::username;
			fstream fs;
			fs.open("fa_info.txt",ios::in|ios::binary);
			fs.seekg(0);
			while(!fs.eof())
			{
				val=-1;
